...
static ssize_t agilent_82357a_generic_write(gpib_board_t *board, uint8_t *buffer, size_t length, int send_commands, int send_eoi)
{
	...
	if(retval == -ETIMEDOUT)
	{
		agilent_82357a_abort(a_priv, 0);
		write_aborted = 1;
		printk("%s: %s: agilent_82357a_send_bulk_msg timed out, bytes_written=%i\n", 
	...
	if(test_bit(AIF_WRITE_COMPLETE_BN, &a_priv->interrupt_flags) == 0 && write_aborted == 0)
	{
		agilent_82357a_abort(a_priv, 0);
		write_aborted = 1;
	}
...
int agilent_82357a_line_status( const gpib_board_t *board )
{
	...
	retval = agilent_82357a_read_registers(a_priv, &bus_status, 1);
	if(retval)
	{
		printk("%s: %s: agilent_82357a_read_registers() returned error\n", __FILE__, __FUNCTION__);
		return 0;
